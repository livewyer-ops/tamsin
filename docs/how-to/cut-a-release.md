...
```sh
git tag -s v0.1.0-rc.1 RELEASE_SHA
...
`ghcr.io/livewyer-ops/tamsin:0.1.0-rc.1`; it does not move `latest`.

Check:

- the GitHub release contains four platform binaries,
  `tamsin-third-party-licenses.tar.gz`, and `SHA256SUMS`;
...
- `tamsin --version` reports `0.1.0-rc.1`;
...
```sh
git tag -s v0.1.0 RELEASE_SHA
git push origin v0.1.0
```

After the same gates pass, the workflow publishes the GitHub release, the
versioned multi-platform image `ghcr.io/livewyer-ops/tamsin:0.1.0`, and then
