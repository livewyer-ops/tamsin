...
func testEncoder(t *testing.T, sink io.Writer) *Encoder {
	t.Helper()
	encoder, err := NewEncoder(sink, testRunID)
	if err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, time.August, 9, 8, 35, 46, 0, time.UTC)
	encoder.now = func() time.Time {
		now = now.Add(time.Millisecond)
		return now
	}
	encoder.started = encoder.now()
	return encoder
}

func emitHello(t *testing.T, encoder *Encoder, limit uint64) {
	t.Helper()
	if err := encoder.Emit(nil, Hello{
...
func TestEncoderWritesOrderedFlushableNDJSON(t *testing.T) {
	...
	if err := encoder.Emit(InputScope(0), InputDeclared{Input: "file:///input.ts"}); err != nil {
		t.Fatal(err)
	}
	if err := encoder.Emit(nil, RunFinished{Outcome: RunSucceeded, ExitCode: 0}); err != nil {
		t.Fatal(err)
	}
...
func TestEncoderEnforcesProcessBoundaries(t *testing.T) {
	...
	if err := encoder.Emit(nil, RunStarted{}); err == nil {
		t.Fatal("stream started without hello")
	}
	emitHello(t, encoder, DefaultMaxEventBytes)
	if err := encoder.Emit(nil, Hello{}); err == nil {
	...
	encoder = testEncoder(t, io.Discard)
	emitHello(t, encoder, DefaultMaxEventBytes)
	if err := encoder.Emit(nil, RunFinished{Outcome: RunFailed, ExitCode: 1}); err != nil {
		t.Fatal(err)
	}
	if err := encoder.Emit(nil, Diagnostic{}); err == nil {
...
func TestEncoderRejectsOversizedRecordWithoutConsumingSequence(t *testing.T) {
	...
	encoder := testEncoder(t, &sink)
	emitHello(t, encoder, 512)
	if err := encoder.Emit(InputScope(0), InputDeclared{Input: string(bytes.Repeat([]byte{'x'}, 1024))}); !errors.Is(err, ErrEventTooLarge) {
		t.Fatalf("oversized error = %v", err)
	}
	if err := encoder.Emit(nil, RunFinished{Outcome: RunFailed, ExitCode: 1}); err != nil {
		t.Fatal(err)
	}
	_, last, _ := bytes.Cut(sink.Bytes(), []byte("\n"))
	var envelope Envelope
...
func TestEncoderSerialisesConcurrentPublishers(t *testing.T) {
	...
	for index := range workers {
		...
		go func() {
			defer group.Done()
			if err := encoder.Emit(InputScope(index), InputDeclared{Input: "input"}); err != nil {
				t.Errorf("Emit() error = %v", err)
			}
		}()
	}
	group.Wait()
	if err := encoder.Emit(nil, RunFinished{Outcome: RunSucceeded}); err != nil {
		t.Fatal(err)
	}
	decoder := json.NewDecoder(bytes.NewReader(sink.Bytes()))
...
func TestDiagnosticIsBoundedAndValidated(t *testing.T) {
	t.Parallel()
	diagnostic, err := NewDiagnostic("config.invalid", strings.Repeat("é", 3000), "fix it", true)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := NewDiagnostic("bad code", "message", "", false); err == nil {
...
func TestEventPayloadJSON(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"run.started", RunStarted{StartedAt: started, Profile: "preserve", ProfileVersion: "1", DryRunMode: "exact", VerificationMode: "auto", Concurrency: pointer(uint64(2)), Transfers: pointer(uint64(4)), RequestedInputs: pointer(uint64(0))},
		...
		{"flow.planned", FlowPlanned{FlowID: "flow", SourceID: "source", Kind: ingest.FlowKindCollection, Root: true, Format: "urn:x-nmos:format:multi"},
			`{"flow_id":"flow","source_id":"source","kind":"collection","root":true,"format":"urn:x-nmos:format:multi"}`},
		{"progress.snapshot", ProgressSnapshot{Revision: 1, Phase: progress.PhaseStore},
		...
		{"diagnostic", Diagnostic{Severity: SeverityError, Code: ingest.FailureCodeConfigInvalid, Message: "Configuration is invalid.", Hint: "Correct it."},
		...
		{"object.result", ObjectResult{ObjectID: "object", Timerange: "[0:0_1:0)", Bytes: 9007199254740993, SHA256: "digest", Disposition: ingest.ObjectDispositionIngested, Verification: ingest.ObjectVerificationVerified, VerificationMethod: ingest.VerificationMethodReadback},
		...
		{"flow.result", FlowResult{FlowID: "flow", SourceID: "source", Kind: ingest.FlowKindEssence, Role: "video", TAMSFlowProfileID: "profile", Disposition: ingest.FlowWritten},
		...
		{"input.finished", InputFinished{Input: "file:///input.ts", Profile: "preserve", ProfileVersion: "1", Status: ingest.ResultStatusFailed, Verification: ingest.VerificationNotReached, ErrorCode: ingest.FailureCodeInterrupted, Message: "Interrupted before dispatch."},
...
func TestEventScopeAndOptionalSettingsJSON(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{InputFinished{Input: "file:///input.ts", Profile: "demux", ProfileVersion: "1", FFmpegVersion: "ffmpeg version 5.1", MediaToolchain: "toolchain", RootFlowID: "flow", Bytes: 42, SHA256: "digest", Status: ingest.ResultStatusIngested, Verification: ingest.VerificationVerified, FlowCount: 3, ObjectCount: 2},
		...
		{FlowPlanned{FlowID: "flow", SourceID: "source", Kind: ingest.FlowKindEssence, Role: "video", ParentFlowID: "parent", Container: "video/mp2t", TAMSFlowProfileID: "profile"},
